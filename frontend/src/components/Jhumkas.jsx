        ...
        <div className='flex flex-col gap-4'>
            ...
            <div className='grid grid-cols-1 gap-8 px-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
                {jumka.slice(0, 8).map((src, i) => (
                    <div key={i} className='cursor-pointer border border-gray-200 rounded-md hover:border-gray-500 transition-all duration-300 ease-in-out overflow-hidden'>
                        <img src={src.image} alt="jewe" className='h-96 w-full rounded-t-md transition-transform duration-500 ease-in-out transform hover:scale-101' />
                        ...
                        <div className='flex'>
                            ...
                            <p className='my-3 ml-5'>{src.price}</p>
                        </div>
                    </div>
                ))
                }
            ...
            <div className='flex justify-center py-8'>
                <button className='bg-green-950 text-white px-8 py-3 rounded-md cursor-pointer'>View all</button>
            </div>
        </div>
    )
}
