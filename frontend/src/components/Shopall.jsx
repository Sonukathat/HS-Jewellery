...
function Shopall() {
  ...
  return (
    <>
      <Header />
      <ToastContainer />
      <Details all={all} heading="Shop-All" headingimage="https://images.unsplash.com/photo-1707222611166-f80ded88b677?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OTZ8fGp1bWtoYSUyMGpld2VsZXJ5fGVufDB8MHwwfHx8MA%3D%3D" handleBuyNow={handleBuyNow} />
      ...
      {showForm && selectedProduct && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <img src={selectedProduct.image} alt={selectedProduct.name} className="h-48 w-full object-cover rounded mb-3" />
            ...
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex justify-between mt-4">
                <button type="button" onClick={() => setShowForm(false)} className="bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500 transition-all">Cancel</button>
                <button type="submit" className="bg-[#A58A6C] text-white px-4 py-2 rounded hover:bg-[#8c6f53] transition-all">Place Order</button>
              </div>
            </form>
          </div>
        </div>
